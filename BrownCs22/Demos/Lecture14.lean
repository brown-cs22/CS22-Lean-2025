...
open BigOperators FinsetInterval BrownCs22
...
Here's the bowling pins example from lecture.
We *don't* expect you to find all of these lemma names like `Finset.uIcc_self` on your own.

-/


example : ∀ n : ℕ, 2 * ∑ i in [[0, n]], i = n*(n + 1) := by
  basic_induction
  { rewrite Finset.uIcc_self
    rewrite Finset.sum_singleton
    numbers }
  { fix n
    assume hn
    rewrite Finset.sum_uIcc_succ
    rewrite mul_add
    rewrite hn
    ring }
...
Here's an example we did in lecture today!
